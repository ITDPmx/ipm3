...
export class ChartSixComponent implements OnInit {
	...
	constructor(private chartService: ChartSixDataService) {
		// this.yearObserver = Observable.create(observer => {
		// 	observer.next(this.updateYear);
	...
	ngOnInit() {
		...
		// .filter(event => {
		// 	return event.years[this._title]
		// });


		if(this._zm && this._series  && this._categories && this._colors){
	...
	ngOnChanges(...args: any[]) {
		// let _observable = this.yearObserver
		// .subscribe(valor => {
		// 	console.log(valor);
		// });
		...
		// 		data: [62,100,87,66,78,86,70,83,47,83,87,70,41,80,53,66,61,67,91,50,51,65,87,72,73,64,86,4,80,69,66,72,35,8,67,70,83,48,76,83,57,85,61,74,98,78,85,91,49,80,75,66,67,68,74,84,61,85,74]
		// 	});
		// 	this._chart["series"][0].name = "Otros";

		// 	this._chart.series[1].update({
		// 		data: [38,0,13,34,22,14,30,17,53,17,13,30,59,20,47,34,39,33,9,50,49,35,13,28,27,36,14,96,20,31,34,28,65,92,33,30,17,52,24,17,43,15,39,26,2,22,15,9,51,20,25,34,33,32,26,16,39,15,26]
		// 	});
		// 	this._chart["series"][1].name = "Movilidad";
		// 	this._chart.redraw();
		// }
